...
function MainScene:onCreate()
	...
	-- local sprite = display.newSprite("Item_pause.png")
    ...
    scoreLabel = cc.Label:createWithSystemFont(score, "Arial", 64)
    :align(display.CENTER, display.right - 100, display.top - 50)
    :addTo(self)
	
    gameOverLabel = cc.Label:createWithSystemFont("GAME OVER!!!", "Arial", 64)
...
function MainScene:onTouch(event, x, y)
	-- print('MainScene >>>>> line = 185', event, x, y);
	if score > 100 then 
		if not isEnd then
			self:beingfool()
			isEnd = true
...
function MainScene:beingfool()
	local str_tip = "叫你玩就玩啊，傻吊";
	gameOverLabel:setVisible(true)
	gameOverLabel:setString("")
	local word_callback = cc.CallFunc:create(function()
		...
		self.schedulerID = scheduler:scheduleScriptFunc(function()  
			...
			if count % 10 == 0 then 
				...
				count = 0
				gameOverLabel:setString(str)
				num = num + 1
			end
			if num > string.len(str_tip) then 
    ...
    self:runAction(cc.Sequence:create(word_callback))
end

function MainScene:runAnimation()
	local sp_1 = cc.Sprite:create("anim/img_1.png");  
	...
	if animation then  
		for i = 1, 30 do  
			local frameName = "anim/img_" .. i .. ".png"  
		...
		sp_1:stopAllActions()  
		sp_1:runAction(cc.RepeatForever:create(cc.Animate:create(animation)))  
	end
end
...
function MainScene:onEnter()
	...
	local function onTouchBegin(touch, event)
		local locationInNode = self:convertToNodeSpace(touch:getLocation())
        return self:onTouch("began", locationInNode.x, locationInNode.y)
    end
    local function onTouchMoved(touch, event)
        local locationInNode = self:convertToNodeSpace(touch:getLocation())
    end
