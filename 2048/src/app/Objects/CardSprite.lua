...
function CardSprite:ctor(num,w,h,cX,cY)
	...
	if num > 0 then
	    ...
	    --     size = 80,
		-- })
		self.numLabel = cc.Label:createWithSystemFont(num, "Arial", 80)
	    ...
	    --     size = 80,
		-- })
		self.numLabel = cc.Label:createWithSystemFont("", "Arial", 80)
    	self.numLabel:setPosition(self.bg:getContentSize().width / 2 - 20, self.bg:getContentSize().height / 2)
	end
...
function CardSprite:setNumber(num)
	...
	if num >=0 then
		...
		self.numLabel:setPosition(self.bg:getContentSize().width / 2, self.bg:getContentSize().height / 2)
	end

	if num >=16 then
		self.numLabel:setSystemFontSize(70)
		self.numLabel:setPosition(self.bg:getContentSize().width / 2 + 5, self.bg:getContentSize().height / 2)
	end

	if num >=128 then
		self.numLabel:setSystemFontSize(60)
		self.numLabel:setPosition(self.bg:getContentSize().width / 2 + 10, self.bg:getContentSize().height / 2)
	end

	if num >=1024 then
		self.numLabel:setSystemFontSize(50)
		self.numLabel:setPosition(self.bg:getContentSize().width / 2 + 15, self.bg:getContentSize().height / 2)
	end
